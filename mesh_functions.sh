...
function substituteVariables(){
	...
	do
		...
		sed -i "s/\$radio0_disable/'${radio0_disable}'/g" "$files"
		sed -i "s/\$radio1_disable/'${radio1_disable}'/g" "$files"
		sed -i "s/\$adhoc0_disable/'${adhoc0_disable}'/g" "$files"
		sed -i "s/\$adhoc1_disable/'${adhoc1_disable}'/g" "$files"
		sed -i "s/\$radio0_ap_disable/'${radio0_ap_disable}'/g" "$files"
		sed -i "s/\$radio1_ap_disable/'${radio1_ap_disable}'/g" "$files"
		sed -i "s/\$radio0_channel/'${radio0_channel}'/g" "$files"
...
function compile_Image(){
	# Compile from source
	rm "${build_dir[$batman_routing_algo]}"/bin/"${platform[$devicetype]}"/"${firmware_name[$devicetype]}"
	cd "${build_dir[$batman_routing_algo]}"
	make -j${nproc} V=s
}
...
function check_Firmware_imagebuilder(){
	...
	echo "${build_dir[$batman_routing_algo]}"/bin/"${platform[$devicetype]}"/"${firmware_name_imagebuilder[$devicetype]}"
